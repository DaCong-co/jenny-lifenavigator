...
def resolve_allowed_path(
    ...
    *,
    ...
    denied_dirs: Iterable[str | Path] | None = None,
    strict: bool = False,
) -> Path:
    ...
    ``denied_dirs`` è una deny-list con precedenza su tutto il resto: un
    percorso dentro una directory negata viene rifiutato anche se cade dentro
    ``allowed_root``, dentro l'allowlist, o sotto ``UNRESTRICTED``. Il
    confronto usa il percorso già risolto (``Path.resolve()``), quindi un
    symlink dentro il workspace che punta dentro una directory negata non
    aggira il confine.
    """
    resolved = _resolve_path(path, workspace, strict=False)
    # deny > allow: checked first, on the resolved path, so nothing below can
    # re-admit a path that lives inside a denied directory.
    for denied in (denied_dirs or []):
        if _is_path_within(resolved, denied, path_resolved=True):
            boundary = _safe_expanduser(denied)
            raise WorkspaceBoundaryError(
                f"Path {path} is inside denied directory {boundary}"
                + WORKSPACE_BOUNDARY_NOTE
            )
