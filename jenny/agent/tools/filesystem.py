...
class _FsTool(Tool):
    ...
    def __init__(
        ...
        denied_dirs: list[Path] | None = None,
    ):
        ...
        # deny-list con precedenza su allow-list: una directory qui dentro non è
        # raggiungibile né in lettura né in scrittura, qualunque sia ``allowed_dir``.
        # È ciò che tiene ``life_model/`` fuori dai tool generici sui file (v.
        # ``security/workspace_policy.resolve_allowed_path``).
        self._denied_dirs = list(denied_dirs or [])
    ...
    def create(cls, ctx: Any) -> Tool:
        ...
        return cls(
            ...
            # La Life Model è accessibile solo dai suoi tool dedicati; i tool
            # generici sui file (read/write/edit/apply_patch/list_dir) non la
            # vedono affatto. Il percorso è risolto dalla deny-list in
            # ``resolve_allowed_path``, quindi anche un symlink non aggira.
            denied_dirs=[Path(ctx.workspace) / "life_model"],
        )
