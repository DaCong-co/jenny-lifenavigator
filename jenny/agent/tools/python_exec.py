...
def _active_path_denied_dirs() -> list[str] | None:
    """Deny-list attiva sul thread corrente: ``life_model/`` sotto il workspace.

    Legge lo slot RAW ``boundary`` e non ``_active_path_boundary()``, che è
    bypass-gated: la deny-list deve valere anche dentro una finestra di
    ``_path_guard_bypass()`` — è lì che la policy stessa risolve un percorso, e
    un symlink dentro il workspace che punta dentro ``life_model/`` non deve
    poter eludere il confine in quella finestra.
    """
    boundary = getattr(_path_guard_state, "boundary", None)
    if boundary is None:
        return None
    return [os.path.join(boundary, "life_model")]
        ...
        *,
        ...
        with _path_guard_bypass():
            ...
            return resolve_allowed_path(
                ...
                allowed_root=root,
                denied_dirs=_active_path_denied_dirs(),
            )
