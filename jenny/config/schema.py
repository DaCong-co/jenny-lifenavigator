...
class LifeReviewConfig(Base):
    """Life review: the periodic, user-facing reflection pass.

    Reads the Life Model and recent events, then decides whether to message the
    user. Defaults to silence. v1 is cron-only (no ``/life_review`` command).
    """

    _HOUR_MS = 3_600_000

    # Opt-in: this is the one internal job that proactively talks to the user,
    # so it does not start on its own the way Dream and the gardener do.
    enabled: bool = False
    interval_h: int = Field(default=168, ge=1)  # weekly
...
class AgentDefaults(Base):
    ...
    life_review: LifeReviewConfig = Field(default_factory=LifeReviewConfig)
