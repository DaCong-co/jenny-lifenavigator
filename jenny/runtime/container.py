...
class GatewayContainer:
    ...
    def build(self) -> None:
        ...
        # Register the Life review system job (opt-in, weekly). v1 cron-only.
        life_review_cfg = config.agents.defaults.life_review
        if life_review_cfg.enabled:
            self.cron.register_system_job(CronJob(
                id="life_review",
                name="life_review",
                schedule=life_review_cfg.build_schedule(),
                payload=CronPayload(kind="system_event"),
            ))
            logger.info("Life review: {}", life_review_cfg.describe_schedule())
        else:
            logger.info("Life review: disabled")
