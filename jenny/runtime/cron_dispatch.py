...
_SYSTEM_WORKERS: tuple[str, ...] = ("dream", "gardener", "life_review")
...
class CronDispatcher:
    ...
    async def _dispatch(self, job: "CronJob") -> str | None:
        ...
        if job.name == "life_review":
            return await self._run_life_review(agent)
    ...
    async def _run_life_review(self, agent: "CronCapableAgent") -> str | None:
        """The weekly, user-facing reflection pass. Opt-in (``enabled=False``).

        Reads the Life Model and recent events, then decides whether to message
        the user. Defaults to silence. Never writes the model.
        """
        from jenny.agent.life_review import run_life_review
        from jenny.config.loader import load_config

        cfg = load_config().agents.defaults.life_review
        if not cfg.enabled:
            logger.debug("Life review: disabled")
            return None
        await run_life_review(agent, agent.context.life_model)
        return None
    ...
    async def _dream_cycle(self, agent: "CronCapableAgent") -> str | None:
        ...
        from jenny.agent.life_model_cycle import run_life_model_pass
        ...
        # Pass B: Life Model maintenance (daily-gated). Additive and independent
        # of the memory cursor/review counters — it never advances the cursor and
        # never talks to the user. A failure here leaves the memory pass intact.
        try:
            await run_life_model_pass(agent, agent.context.life_model)
        except Exception:
            logger.exception("Life Model pass failed")
